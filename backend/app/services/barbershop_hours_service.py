...
def get_working_hours(barbearia) -> dict[str, dict[str, str | bool]]:
    return normalize_working_hours(getattr(barbearia, "horarios_funcionamento", None))


def get_barbeiro_working_hours(barbearia, barbeiro) -> dict[str, dict[str, str | bool]]:
    base_schedule = get_working_hours(barbearia)
...
def get_working_window(barbearia, target_date: date, barbeiro=None) -> tuple[time, time] | None:
    base_schedule = get_working_hours(barbearia)
    ...
    barbeiro_window = _get_window_from_schedule(
        get_barbeiro_working_hours(barbearia, barbeiro),
        target_date,
    )
...
def is_within_working_hours(barbearia, start_at: datetime, end_at: datetime, barbeiro=None) -> bool:
    window = get_working_window(barbearia, start_at.date(), barbeiro=barbeiro)

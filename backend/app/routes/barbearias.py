...
def _audit_tenant_action(
    ...
    create_admin_audit_log(
        ...
        establishment_id=barbearia.id,
        action=action,
        entity_type="establishment",
        entity_id=barbearia.id,
        ip_address=request.client.host if request.client else None,
        user_agent=request.headers.get("user-agent"),
        metadata={
            "result": "success",
            "correlation_id": request.headers.get("x-request-id"),
            "status": barbearia.status_manual,
            "plan": barbearia.plano,
        },
    )
...
def criar(
    ...
    _audit_tenant_action(db, request, claims, barbearia, "tenant_account_created")
    return barbearia
...
def atualizar(
    ...
    barbearia.auth_version = int(barbearia.auth_version or 0) + 1
    ...
    db.commit()
    db.refresh(barbearia)
    _audit_tenant_action(db, request, claims, barbearia, "tenant_account_updated")
    return barbearia
...
def remover(
    ...
    barbearia.status_manual = "inativo"
    barbearia.auth_version = int(barbearia.auth_version or 0) + 1
    _audit_tenant_action(db, request, claims, barbearia, "tenant_account_disabled")
